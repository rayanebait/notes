...
$\m_E$ est le seul idéal de valuation $0$. Il y'a quand
même des idéaux qui ne sont pas de la forme $\alpha.\m_E$
si $\sup|I|$ est pas dans $|E|$. Même si $(|y_n|)_n$ tend
vers $a$ y'a pas de raisons pour que $(y_n)$ soit Cauchy.
...
\subsection{Valeur absolue d'un corps perfectoide}
On a 
\begin{enumerate}
...
\[\pi\colon \Or_E^\flat\to \Or_E/p\]
et on prouve aussi naturellement que le noyau est $(t)$.
Si $x\in \ker(\pi)$ alors $|x|\leq |p|=|t|$ d'où
$x/t\in \Or_E^\flat$ puis $x=(x/t).t\in (t)$.
On a montré que 
\[\Or_{E^\flat}/t\simeq \Or_E/p\]
\begin{rem}
    Pas vraiment, en fait j'ai triché en montrant pas
    que $\Or_E^\flat$ est de valuation. Il faudrait lift
    $x$ en $x\in (\Or_E)^{perf}$ et successivement approximer
    en utilisant le fait que $\Or_E$ est lui de valuation.
\end{rem}
...
noyau. La preuve est par approximations successives.
Voir la sous-section d'après.
\begin{rem}
    ...
    $[t]-p=\ker(\theta_{\C_p})$.
\end{rem}
\subsubsection{$\ker(\theta_E)$ est principal}
Si on prends $x\in \ker(\theta_E)$ tel que $|x_1|=1$
et $y\in\ker(\theta_E)$ on peut supposer que 
\[y=xz_m+p^mc.\]
Pour $m=0$, on prends $z_0=[y_0/x_0]$ (on voit $|x_0|=1$)
puis pour $m\geq 1$ on a 
\[\theta_E(y-xz_m)=0\]
d'où $c\in\ker(\theta_E)$ puis $c=xd_0+p.e$ et en
remplaçant l'approximation voulue. La suite $(z_n)_n$
converge facilement !

\subsubsection{Topologie de $A_{\inf}$}
On prend les vecteurs de Witt d'un anneau parfait de 
caractéristique $p$ qui est parfait (mais pas d'un corps). Ducoup 
on obtient un anneau de valuation (mais pas forcément discrète)
$p$-adiquement complet. 

Dans l'idée la complétude provient de l'écriture
$\sum_{i=1}^{+\infty}p^i[a_i]$ qui elle même provient de la 
perfection (je crois). La valuation est
donnée par $i_0.v_p(a_{i_0})$ pour le premier indice non nul ?
